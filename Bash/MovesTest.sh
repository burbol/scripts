...
for i in 11 17 66 # OH density of the SAM 
do
...
# Now we determine the simulation's box size and then we divide the x- and y-coordinates by 2 to get the center of the box, which is also the center of the surface  
      ...
      # Now we have to check out if the distance we have to move the y-coordinate is negative. If so we change the format so that GROMACS can read it     
...
# We need to change the format of these numbers and put them in absolute value in order to check if they are small enough 
 ...
 # First for x
        if [ $(echo "$diffx <1"|bc) = 1 ]        
           ...
           then z=$diffx
           diffx=$(echo "0$z")
           echo "This is our new formatted diffx: $diffx" >> output${k}.dat
           fi
       fi
       ...
       if [ $(echo "$diffx <0"|bc) = 1 ]        
           ...
           diffx=$(echo "0$z")
              echo "This is our new formatted diffx in absolute value: $diffx" >> output${k}.dat
           fi
       fi    
 #Now for y
    ...
    if [ $(echo "$diffy < 1"|bc) = 1 ]        
       then    if [ $(echo "$diffy>0 "|bc) = 1 ] ## Checking if number is between 0 and 1
          then  z=$diffy
           diffy=$(echo "0$z")
           echo "This is our new formatted diffy: $diffy" >> output${k}.dat
           fi
    fi
      ...
      if [ $(echo "$diffy < 0"|bc) = 1 ]        
           ...
           diffy=$(echo "0$z")
              echo "This is our new formatted diffx in absolute value: $diffy" >> output${k}.dat
           fi
      fi
         ...
         if [ $(echo "$diffx < 0.001"|bc) = 1 ]
           then
           echo "This is the value of diffx inside the if statement: $diffx" >> output${k}.dat
           echo "New x coordinate $movedx is equal to the surface center at $newx" >> output${k}.dat
           else echo "Error! New x coordinate $movedx is not equal to the surface center at $newx" >> output${k}.dat
           echo "See output$k file! for sam$i_water$j!">Error${i}pc_${j}_${k}ns_${l}ns.dat
         fi
         if [ $(echo "$diffy < 0.001"|bc) = 1 ]
           then 
           echo "This is the value of diffy inside the if statement: $diffy" >> output${k}.dat
           echo "New y coordinate $movedy is equal to the surface center at $newy" >> output${k}.dat
           else echo "Error! New y coordinate $movedy is not equal to the surface center at $newy" >> output${k}.dat
           echo "See output$k file! for sam$i_water$j!">Error${i}pc_${j}_${k}ns_${l}ns.dat
         fi
...
#        echo 0 | /home/shavkat/GMX/bin/trjconv -f NVT_sam${i}_water${j}.xtc -s NVT_sam${i}_water${j}.tpr -trans ${x} ${y} 0 -o dummy_${k}ns_${l}ns.gro -pbc atom -b ${start} -e ${ending}
        ...
        echo 1 4 4 | /home/shavkat/GMX/bin/g_densmap -f dummy_${k}ns_${l}ns.xtc -s NVT_sam${i}_water${j}.tpr  -n index${i}_${j}.ndx -amax 5 -rmax 5 -bin 0.05 -od densmap_${i}pc_${j}_${k}ns_${l}ns.dat 
