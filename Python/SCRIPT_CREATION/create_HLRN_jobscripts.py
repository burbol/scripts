...
NodeType = 'mpp2' #mpp1, smp1, data, prepost1, test, ccm1 
	...
	for j in molec: 
		...
		for k in range(N):
			...
			if k == 0:
				...
				f1.write('aprun -n 2 grompp -f ' + Minimdp + ' -c ' + Startfile + '.gro -p ' + topfile + ' -o '+ Minifile +'.tpr -maxwarn 1\n')

				f1.write('aprun -n ' + str(Totalcpus) + ' mdrun -deffnm ' + Minifile + ' -maxh ' + str(SimTime) + '\n')
				
				f1.write('aprun -n 2 grompp -f ' + NVTmdp + ' -c ' + Minifile + '.gro -p ' + topfile + ' -o '+ NVTfile +'.tpr -maxwarn 1\n')
				
				f1.write('aprun -n ' + str(Totalcpus) + ' mdrun -deffnm ' + NVTfile +  ' -tunepme -v -cpo '+ NVTfile + '.cpt  -maxh ' +str(SimTime) + '\n')
			else:
				
				f1.write('aprun -n ' + str(Totalcpus) + ' mdrun -deffnm '+ NVTfile + ' -s ' + NVTfile + '.tpr -tunepme -v -cpi  '+ NVTfile + '.cpt -maxh ' + str(SimTime) + '\n')
