...
# submit command for test: example:
# "msub -l feature=mpp1:test -l walltime=00:30:00 -l nodes=4:ppn=24 s21_w1000_testcuda"
#For an interactive job use:
#msub -I -l feature=mpp2 -l walltime=00:30:00 -l nodes=4:ppn=24 
#msub -I -l feature=smp1:test -l walltime=00:20:00 -l nodes=1:ppn=32 #only in Hannover for non-parallel jobs
...
Walltime = '00:30:00'  # Walltime in format 'h:mm:ss'
SimTime = 0.25   # maximum run length of simulation in hours, it can be also a fraction of an hour
...
NodeType = 'test' #mpp1, smp1, data, prepost1, test, ccm1 
...
for i in pc:
	for j in molec: 
		...
		Scriptname = 's'+str(i)+'_w'+str(j)+'_testcuda'
		Jobname = 's'+str(i)+'_w'+str(j) # Name of job
		Indexfile = 'index'+str(i)+'_'+str(j)
		
		#os.system('mkdir '+Jobname )
		
		N = 2  # NUMBER OF SCRIPTS TO SUBMIT: 1 scripts each ns 
		Totalcpus=NodesNum[j]*CpuNum

		for k in range(N):
		
			f1 = open(Dir + '/' + Jobname +'/'+ Scriptname ,'w+') #no numbering needed because we want only 1 script
			...
			#The next options should be given in the submitting command. For example:
			# "msub -l feature=mpp1:test -l walltime=00:30:00 -l nodes=4:ppn=24 s21_w1000_testcuda"
			...
			if k == 0:
				...
				f1.write('grompp -f ' + Minimdp + ' -c ' + Startfile + '.gro -p ' + topfile + ' -o '+ Minifile +'.tpr -maxwarn 1\n')

				f1.write('aprun -n ' + str(Totalcpus) + ' mdrun -deffnm ' + Minifile + ' -maxh ' + str(SimTime) + '\n')
				
				f1.write('grompp -f ' + NVTmdp + ' -c ' + Minifile + '.gro -p ' + topfile + ' -o '+ NVTfile +'.tpr -maxwarn 1\n')
				
				f1.write('aprun -n ' + str(Totalcpus) + ' mdrun -deffnm ' + NVTfile +  ' -cpo '+ NVTfile + '.cpt  -maxh ' +str(SimTime) + ' -tunepme -v -testverlet \n')
			else:
