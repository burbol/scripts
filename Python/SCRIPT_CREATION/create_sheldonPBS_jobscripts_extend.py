...
Nodes = {1000:2, 6500:1, 9000:1, 4000:1, 5000:1, 10000:1,7000:1, 8000:1} 
...
for i in pc:
	for j in molec: 
		...
		Scriptname2 = 's'+str(i)+'_w'+str(j)+'_new'
		...
		for k in range(N):
			...
			if k == 0:
				...
				##JobOut.write('mdrun -nt '+ str(CpuNum*NodesNum)+' -cpi '+ Filename + '.cpt -s ' + Filename + '_next.tpr -maxh ' +str(SimTime) + ' -v -testverlet\n')
				JobOut.write('mdrun -nt '+ str(CpuNum*NodesNum)+' -cpi '+ Filename + '.cpt -s ' + Filename + '.tpr -maxh ' +str(SimTime) + ' -deffnm '+Filename+ ' -v -testverlet\n')
				...
				JobOut.write( 'RUNTIME=$(($(date +%s)-$STARTTIME))\n' + '\n'+ 'echo \"the job took $RUNTIME seconds...\"\n' + '\n' + 'if [[ $RUNTIME -lt 10 ]]; then\n' +
							...
							'   echo "job took less than 10 seconds to run, calling _new jobscript"\n' +'    sbatch '  + Scriptname2 + str(0) + '\n' +'   exit\n' + 'else\n' + '   echo "everything fine..."\n' + '   sbatch ' + Scriptname + str(k+1) + '\n') 
