...
pc = [33]
#molec = [1000, 2000, 3000, 5000] 
molec = [4000] # FOR TESTING!!

BackupDir = '/home/eixeres/finished/'   # directory to copy (backup) simulation output

for i in pc:
	for j in molec:
		
		#Dir = './sheldon-ng/s'+str(i)+'_w'+str(j) # directory to save script
		Dir = '/Users/burbol/Desktop/scripts/Python/SCRIPT_CREATION/scheldon-ng/s'+str(i)+'_w'+str(j)
		...
		#Scriptname = 'test_s'+str(i)+'_w'+str(j) # FOR TESTING!!
		...
		for k in range(N):
			...
			if k == 0:
				...
				JobOut.write('mpirun -np ' + str(CpuNum) + ' mdrun -s ' + Filename + '_next.tpr -deffnm   '+ Filename + ' -maxh ' +str(SimTime) + '\n')

			else:

				JobOut.write('mpirun -np ' + str(CpuNum) + ' mdrun -s ' + Filename + '.tpr -deffnm   '+ Filename + ' -maxh ' +str(SimTime) + '\n')
				print('mpirun -np ' + str(CpuNum) + ' mdrun -s ' + Filename + '.tpr -deffnm   '+ Filename + ' -maxh ' +str(SimTime) + '\n')
