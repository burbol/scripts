...
myfile1 = open('SAMPeaks1.txt', 'w')
print >> myfile1, '{0}  {1}'.format('File', 'Last SAM\'s Peak')
myfile = open('SAMPeaks.txt', 'w')
print >> myfile, '{0}'.format('Last SAM\'s Peak Position')

pp = PdfPages('/Users/burbol/Downloads/global_SAMS_densmaps/SAMs_densplots.pdf')
...
for b in [0, 5, 11, 17]:
    for c in [1000, 2000, 3000, 4000, 5000, 6500, 7000, 8000, 9000, 10000]:
        ...
        print >> myfile1, '             '
#        print >> myfile, '             '
        filename = 'g_density_SAM_sam%d_water%d.xvg'%(b,c, ) 
        xvg.read(filename)
        input = xvg.array
        x = input[0]
        y = input[1]
